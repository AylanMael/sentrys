...
export async function POST(req: NextRequest) {
  try {
    ...
    const siteContract = scQ.docs[0].data() as Record<string, unknown>;
    ...
    for (const v of vacations) {
      ...
      // DEBUG TEMP (remove after): log first vacation segmentation
      if (vacationIds.length === 1) {
        console.log("[SEG_DEBUG]", {
          vacationId: v.id,
          start: v.startAt.toDate().toISOString(),
          end: v.endAt.toDate().toISOString(),
          segs: segs.map((s) => ({
            code: s.code,
            minutes: s.minutes,
